...
public class RangeController extends FormViewControllerBase {
    @Autowired
    ...
    public ModelAndView view(@RequestBody RangeRequestModel data) {
        ...
        if (data.getParam() != null && data.getParam().size() > 0) {
            ...
            data.setMode("edit");
            fields = this.dataService.buildModel(table, this.dataService.selectRecordByKey(tableName, data.getParam()));
        } else {
        ...
        RangeFormModel form = new RangeFormModel(action, data.getParent(), data.getMode(), tableName, groupCols, filter,
                detail, cols, children);
    ...
    public ActionResult Edit(@RequestBody RangeRequestModel data) {
        ...
        if (!table.getValid_date_cols().isEmpty()) {
            ...
            if (!isExcute) {
                ...
                ret = updateRecord(form.getTableName(), data.getValidDate(), table.getValid_date_cols(),
                        databaseDataMap);
            }
    ...
    private int updateRecord(String tableName, Date validDate, String validDatacols, Map<String, Object> data) {
        ...
        if (hasValidData) {
            ...
            if (validData.length != 1) {
                if (validData.length > 1) {
                    where = "'" + Common.convertToDateText(validDate) + "'" + " between " + validData[0] + " and "
                            + validData[1];
                }
            }
        }
    ...
    private String dateRecursiveCheck(String tableName, String groupCols, Map<String, Object> data) {
        ...
        List<Map<String, Object>> records = this.dataService.selectRecordsByValues(tableName, columnsNeeded, valueMap,
                where);
