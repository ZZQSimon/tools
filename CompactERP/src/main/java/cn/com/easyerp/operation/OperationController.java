...
public class OperationController extends FormViewControllerBase {
    ...
    public static final String OPERATION_EMPTY_RECORD_ID = "_BLANK_";
    @Autowired
    ...
    public ModelAndView dialog(@RequestBody OperationDialogRequestModel request) {
        ...
        UrlInterfaceDescribe urlInterfaceDescribe = (UrlInterfaceDescribe) this.cacheService.getUrlInterface()
                .get(((ApiDescribe) trigger.getApiMap().get(request.getEvent_id())).getId());
        ...
        OperationFormModel form = new OperationFormModel(ActionType.operate, request.getParent(), trigger,
                request.getColumn(), this.dataService.buildModel(table, data, false), apiTableName);
    ...
    private Map<String, Object> buildActionLogData(OperationRequestModel request, TableDescribe table, String key,
            String statusOld, String statusNew) {
        Map<String, Object> ret = new HashMap<String, Object>(8);
        ret.put("table_id", (table.getTable_type() == 1) ? table.getView_main_table() : table.getId());
    ...
    public ActionResult exec(@RequestBody final OperationRequestModel request, final HttpServletRequest httpRequest,
            final AuthDetails user) {
        final String logId = this.logService.getMaxId();
        this.logService.logTemp(logId, LogType.operate, AuthService.getCurrentUserId());
        ...
        for (final Map.Entry<String, ViewDataMap> viewEntry : request.getParam().getData().entrySet()) {
            ...
            this.authService.optionAuth(user, table.getId(), api.getTable_action_id(),
                    (owner == null) ? null : owner.getId(), ids);
            if (api != null) {
                ...
                if (Common.isViewDataMap((DataMap) data)) {
                    apiData = this.viewService.convertToDBValues(this.dataService.getTableDesc(api.getId()),
                            (ViewDataMap) apiData);
                }
            ...
            final TableDescribe opTable = (table.getTable_type() == 1)
                    ? this.dataService.getTableDesc(table.getView_main_table())
                    : table;
            final Map<String, Object> insertRecordParam = new HashMap<String, Object>();
            final Map<String, Object> map2 = this.buildActionLogData(request, table,
                    (valueMap.get(opTable.getIdColumns()[0]) == null) ? null
                            : valueMap.get(opTable.getIdColumns()[0]).toString(),
                    null, null);
        ...
        for (int j = 0; j < execupdateRecordParams.size(); ++j) {
            final Map<String, Object> updateRecordParam = this
                    .updateRecordDaramToDbValue(execupdateRecordParams.get(j));
        ...
        for (int k = 0; k < execinsertRecordParams.size(); ++k) {
            final Map<String, Object> insertRecordParam2 = this
                    .insertRecordDaramToDbValue(execinsertRecordParams.get(k));
        ...
        if (!apiResult.isSuccess()) {
            Common.rollback();
            this.logService.updateLogException(logId, table.getId(), operationId,
                    this.dataService.getMessageText("operation failed", new Object[0]));
            return (ActionResult) new ApiActionResult(apiResult.getResult(),
                    this.dataService.getMessageText("operation failed", new Object[0]), apiResult.getMessages());
        }
        this.logService.updateLogNormal(logId, table.getId(), operationId);
        return (ActionResult) new ApiActionResult(0,
                this.dataService.getMessageText("operation success", new Object[0]), (List) details,
                (Object) "success");
    }
    ...
    @SuppressWarnings({ "unchecked", "rawtypes" })
    public ApiResult execApi(List<Map<String, Object>> execApiParams, List<Map<String, Object>> execupdateRecordParams,
            List<Map<String, Object>> execinsertRecordParams, ApiDescribe api) {
            ...
            for (int j = 0; j < values.size(); j++) {
                if (j == values.size() - 1) {
                    sql = sql + ((ColumnValue) values.get(j)).getId() + " = '"
                            + ((ColumnValue) values.get(j)).getValue() + "'";
                } else {
                    sql = sql + ((ColumnValue) values.get(j)).getId() + " = '"
                            + ((ColumnValue) values.get(j)).getValue() + "',";
                }
    ...
    private List<String> apiSQLToDb(List<Map<String, Object>> execApiParams) {
        ...
        for (int i = 0; i < execApiParams.size(); i++) {
            ...
            UrlInterfaceDescribe urlInterfaceDescribe = (UrlInterfaceDescribe) this.cacheService.getUrlInterface()
                    .get(((ApiDescribe) paramMap.get("api")).getId());
