   ...
   extends FormViewControllerBase
 {
   ...
   public ActionResult selectTableDeploy(AuthDetails auth) {
     ...
     for (Map.Entry<String, TableDescribe> entry : tableDescCache.entrySet()) {
       ...
       if (auth.getId().equals("Super") && (
         (TableDescribe)entry.getValue()).getTable_type() == 0) {
       ...
       if (((TableDescribe)entry.getValue()).getParent_id() != null && 
         ...
         TableDescribe parentTable = this.cacheService.getTableDesc(((TableDescribe)entry.getValue()).getParent_id());
         if (parentTable == null) {
           throw new ApplicationException("parent table :" + ((TableDescribe)entry.getValue()).getParent_id() + " does not exists");
         }
         if (((TableDescribe)entry.getValue()).getTable_type() != 0 && ((TableDescribe)entry
           .getValue()).getTable_type() != parentTable.getTable_type()) {
