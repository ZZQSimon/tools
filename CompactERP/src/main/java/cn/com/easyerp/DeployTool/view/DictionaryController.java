...
public class DictionaryController extends FormViewControllerBase {
    ...
    public ActionResult initDic(AuthDetails user) {
        ...
        List<DictionaryDetails> dictionaryList = this.dictionaryDao
                .selectDictionary(AuthService.getCurrentUser().getLanguage_id(), user.getId());
        List<DictionaryDetails> dictionaryToOne = this.dictionaryDao
                .dictionaryById(((DictionaryDetails) dictionaryList.get(0)).getDic_id(), user.getId());
    ...
    public ActionResult retrieveDic(@RequestBody DictionaryRequestModel request, AuthDetails user) {
        ...
        if ("".equals(retrieveValue) || retrieveValue == null) {
            dictionaryList = this.dictionaryDao.selectDictionary(AuthService.getCurrentUser().getLanguage_id(),
                    user.getId());
        } else {
            dictionaryList = this.dictionaryDao.retrieveDictionary("%" + retrieveValue + "%",
                    AuthService.getCurrentUser().getLanguage_id(), user.getId());
        }
        ...
        for (int i = 0; i < dictionaryAddList.size(); i++) {
            ...
            for (int j = 0; j < dictionaryList.size(); j++) {
                ...
                if (j == 0) {
                    ...
                    dictionary.setKey_international(
                            ((DictionaryAddDetails) dictionaryAddList.get(i)).getDicId().toLowerCase());
                ...
                ((DictionaryAddDetails) dictionaryAddList.get(i))
                        .setDicId(((DictionaryAddDetails) dictionaryAddList.get(i)).getDicId());
                dictionary.setSeq(j + 1);
                dictionary.setId_international(
                        ((DictionaryAddDetails) dictionaryAddList.get(i)).getDicId().toLowerCase());
