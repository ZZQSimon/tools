...
public class DashboardService {
    ...
    public void saveDashBoard(DashboardDescribe dash) {
        this.dashDao.addDashBoard(dash);
    }
    ...
    public void saveSubscribe(DashboardDescribe dash) {
        ...
        for (String user : users) {
            this.dashDao.addSubscribe(dash.getDashboard_id(), DashboardDescribe.getUUID(), user, subscribe_status);
        }
    }

    public List<DashboardDescribe> getDashboards() {
        return this.dashDao.getDashboards();
    }
    ...
    public int getTableCount(String table, String where) {
        return this.dashDao.getTableCount(table, where);
    }

    public List<DatabaseDataMap> getChartList() {
        return this.dashDao.getChartList();
    }
    ...
    public boolean updateSubscribes(List<SubscribeDescribe> subscribes, AuthDetails subscriber) {
        ...
        for (SubscribeDescribe subscribe : subscribes) {
            if (this.dashDao.existTheSubscribe(subscriber, subscribe.getDashboard_id()) != 0) {
                this.dashDao.updateSubscribe(subscribe, subscriber.getId());
                continue;
            }
            this.dashDao.addSubscribe(subscribe.getDashboard_id(), DashboardDescribe.getUUID(), subscriber.getId(),
                    "1");
        }
    ...
    public List<Map<String, Object>> getSubscribes() {
        return this.dashDao.getSubscribes();
    }

    public int isExistsDashboard(DashboardDescribe dash) {
        return this.dashDao.isExistsDashboard(dash);
    }

    public void updateDashboard(DashboardDescribe dash) {
        this.dashDao.updateDashboard(dash);
    }
    ...
    public void cancelSubscribe(String subscribe_id) {
        this.dashDao.cancelSubscribe(subscribe_id);
    }
    ...
    public void addUserDashboard(AuthDetails user) {
        if (this.dashDao.existSubscribe(user) == 0) {
            ...
            for (DashboardDescribe dash : dashs) {
                ...
                this.dashDao.addSubscribe(dash.getDashboard_id(), DashboardDescribe.getUUID(), user.getId(),
                        subscribe_status);
            }
        }
    }
    ...
    public void buildDashMenu(DatabaseDataMap dash) {
        ...
        try {
            final Map<String, Object> map = (Map<String, Object>) Common.fromJson(
                    ((HashMap<String, Object>) dash).get("dashboard_param").toString(),
            ...
            if (map.get("menus") == null) {
                ...
                for (int i = 0; i < ((List<String>) map.get("menus")).size(); ++i) {
                    final MenuModel menu = this.cacheService
                            .getMenu(((List<String>) map.get("menus")).get(i).toString());
                    if (this.authService.newAuth(AuthService.getCurrentUser(), this.cacheService.getMenuAuthGroup(), 1,
                            menu)) {
                        menuParam.add(menu);
                    }
                }
