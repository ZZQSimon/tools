...
public class TimingTaskController extends FormViewControllerBase {
    @Autowired
    ...
    public ModelAndView timingTask(@RequestBody TimingTaskRequestModel request) {
        ...
        for (int i = 0; i < timingTask.size(); i++) {
            if (!"".equals(((TimeTaskDescribe) timingTask.get(i)).getInternational_id())
                    && ((TimeTaskDescribe) timingTask.get(i)).getInternational_id() != null) {
                I18nDescribe name_i18n = this.cacheService
                        .getMsgI18n(((TimeTaskDescribe) timingTask.get(i)).getInternational_id());
                ((TimeTaskDescribe) timingTask.get(i)).setName_i18n(name_i18n);
            }

            List<TimeTaskEventDescribe> timeTaskEvent = this.timingTaskDao
                    .selectTimeTaskEvent(((TimeTaskDescribe) timingTask.get(i)).getTask_id());

            List<TimeTaskSysTimeDescribe> timeTaskSysTime = this.timingTaskDao
                    .selectTimeTaskSysTime(((TimeTaskDescribe) timingTask.get(i)).getTask_id());

            List<TimeTaskBusinessTimeDescribe> timeTaskBusinessTime = this.timingTaskDao
                    .selectTimeTaskBusinessTime(((TimeTaskDescribe) timingTask.get(i)).getTask_id());
        ...
        for (TimeTaskDescribe timeTaskDescribe : saveTimingTask) {
            ...
            if (timeTaskDescribe.getSaveType() == 1) {
                ...
                if (timeTaskDescribe.getTimeTaskSysTimeDescribes().size() > 0) {
                    List<TimeTaskSysTimeDescribe> timeTaskSysTimeDescribes = timeTaskDescribe
                            .getTimeTaskSysTimeDescribes();
                ...
                if (timeTaskDescribe.getTimeTaskBusinessTimeDescribes().size() > 0) {
                    List<TimeTaskBusinessTimeDescribe> timeTaskBusinessTimeDescribes = timeTaskDescribe
                            .getTimeTaskBusinessTimeDescribes();
