...
public class TableDeployService {
    ...
    public static final String APPROVE_STATUS_DICT = "approve_status";
    public static final String APPROVE_STATUS_DICT_DEFAULT = "1";
    @Autowired
    ...
    private void saveTableApproveEvent(AddTableDeploy tableDeploy, String isDelete) {
        ...
        if (isDelete == null && tableDeploy.getApproveEvent() != null) {
    ...
    private void updateTable(AddTableDeploy tableDeploy) {
        tableChange(tableDeploy);
    }
    ...
    private void createTable(AddTableDeploy tableDeploy) {
        ...
        if (tableDeploy.getTable_type() == 0 || tableDeploy.getTable_type() == 4 || tableDeploy.getTable_type() == 5)
    ...
    private void deleteTable(AddTableDeploy tableDeploy) {
        ...
        if (tableDeploy.getDeleteTrigger() != null && tableDeploy.getDeleteTrigger().size() != 0) {
            for (int i = 0; i < tableDeploy.getDeleteTrigger().size(); i++) {
                deleteTrigger((TriggerDeploy) tableDeploy.getDeleteTrigger().get(String.valueOf(i)));
            }
        }
    ...
    public String buildColumnSQL(ColumnDescribe column) {
        ...
        case 6:
            return "`" + column.getColumn_name() + "` blob ";
        case 1:
            return "`" + column.getColumn_name() + "` varchar("
                    + ((column.getMax_len() == null || column.getMax_len().intValue() == 0) ? "20"
                            : column.getMax_len())
                    + ")";
        case 15:
        ...
        case 14:
            return "`" + column.getColumn_name() + "` varchar("
                    + ((column.getMax_len() == null || column.getMax_len().intValue() == 0) ? "200"
                            : column.getMax_len())
                    + ")";
        case 2:
        case 5:
        ...
        case 3:
            ...
            MaxLen = (column.getMax_len() == null || column.getMax_len().intValue() == 0) ? 10
                    : column.getMax_len().intValue();
    ...
    private void updateCheckRule(List<TableCheckRuleDescribe> checkRules, String tableName) {
        ...
        if (checkRules != null && checkRules.size() > 0) {
            for (int i = 0; i < checkRules.size(); i++) {
                if ("".equals(((TableCheckRuleDescribe) checkRules.get(i)).getError_msg_id())
                        || ((TableCheckRuleDescribe) checkRules.get(i)).getError_msg_id() == null) {
    ...
    private void addAutoGen(Map<String, AutoGenDeploy> autoGens, String tableName) {
        ...
        for (Map.Entry<String, AutoGenDeploy> autoGen : autoGens.entrySet()) {
            ...
            if ("".equals(((AutoGenDeploy) autoGen.getValue()).getInternational_id())
                    || ((AutoGenDeploy) autoGen.getValue()).getInternational_id() == null) {
    ...
    private void createCondition(List<ActionPrerequistieDescribe> condition, String actionId) {
        ...
        for (int i = 0; i < condition.size(); i++) {
            ...
            if ("".equals(((ActionPrerequistieDescribe) condition.get(i)).getViolate_msg_international_id())
                    || ((ActionPrerequistieDescribe) condition.get(i)).getViolate_msg_international_id() == null) {
    ...
    private void deleteApi(String actionId) {
        this.tableDeployDao.deleteApi(actionId);
    }
    ...
    private void updateColumn(AddTableDeploy tableDeploy) {
        ...
        if (tableDeploy.getDeleteColumns() != null && tableDeploy.getDeleteColumns().size() != 0) {
            ...
            for (int i = 0; i < deleteColumns.size(); i++) {
                ...
                if (tableDeploy.getTable_type() == 0 || tableDeploy.getTable_type() == 4
                        || tableDeploy.getTable_type() == 5) {
                    String columnDBSQL = buildDeleteColumnDBSQL((ColumnDeploy) deleteColumns.get(i),
                            tableDeploy.getId());
                    if (columnDBSQL != null) {
    ...
    private String buildAddColumnDBSQL(ColumnDeploy column, String tableName) {
        if (tableName == null || "".equals(tableName) || column.getColumn_name() == null
                || "".equals(column.getColumn_name()))
            return null;
