...
import cn.com.easyerp.storage.StorageService;
 ...
 public class PrintExcelService {
   ...
   private void createMonthRow(HSSFWorkbook workbook, HSSFSheet sheet, List<String> allMonth, List<Map<String, Object>> selectColumn) {
     ...
     for (int i = 0; i < allMonth.size(); i++) {
       ...
       CellRangeAddress range = new CellRangeAddress(0, 0, selectColumn.size() + 4 * i, selectColumn.size() + 4 * i + 3);
   ...
   private void createHeadRow(HSSFWorkbook workbook, HSSFSheet sheet, List<String> allMonth, List<Map<String, Object>> selectColumn) {
   ...
   private void createDataRow(HSSFWorkbook workbook, HSSFSheet sheet, List<CustomData> data, List<String> allMonth, List<Map<String, Object>> selectColumn) {
     String tableName = CustomReportController.FILTER_TABLE;
     TableDescribe table = this.cacheService.getTableDesc(tableName);
     String dic_id = table.getColumn("Leaving Type").getDic_id();
     ...
     for (CustomData customData : data) {
       ...
       for (int i = 0; i < selectColumn.size(); i++) {
         String language_id, text; I18nDescribe keyI18N; HSSFCell cellData = dataRow.createCell(i);
       ...
       for (int i = 0; i < allMonth.size(); i++) {
         if (customData.getCustomDataDescribes() == null) {
           ...
           CustomDataDescribe monthData = (CustomDataDescribe)customData.getCustomDataDescribes().get(allMonth.get(i));
           if (monthData == null) {
             currentCellNum += 4;
           } else {
   ...
   private void createTotalRow(HSSFWorkbook workbook, HSSFSheet sheet, int row, List<String> allMonth, List<Map<String, Object>> selectColumn) {
     HSSFCellStyle rateStyle = rateStyle(workbook);
   ...
   private void createPicture(HSSFWorkbook workbook, HSSFSheet sheet, List<String> allMonth, List<CustomData> data) {
     ...
     if (((CustomData)data.get(0)).getEmployeeType() == null || "".equals(((CustomData)data.get(0)).getEmployeeType())) {
     ...
     JFreeChart chart = ChartFactory.createLineChart("鍥捐〃鏍囬", "鏈堜唤", "绂昏亴鐜�", dataset, PlotOrientation.VERTICAL, true, true, true);
     ...
     //HSSFClientAnchor anchor = new HSSFClientAnchor(0, 0, 0, 0, 0, (short)(data.size() + 10), 12, (short)(data.size() + 50));
   ...
   private List<String> buildX(List<String> allMonth) { return allMonth; }
 
 
   
   private List<Double> buildPictureData(List<String> allMonth, List<CustomData> data) { return buildPictureData(allMonth, data, null); }
   
   private List<Double> buildPictureData(List<String> allMonth, List<CustomData> data, String type) {
     Map<String, Double> dataMap = new HashMap<String, Double>();
     for (CustomData customData : data) {
       if (type != null && 
         !type.equals(customData.getEmployeeType())) {
         continue;
       }
       ...
       for (Map.Entry<String, CustomDataDescribe> entry : customDataDescribes.entrySet()) {
         if (dataMap.get(entry.getKey()) == null) {
           dataMap.put(entry.getKey(), ((CustomDataDescribe)entry.getValue()).getAttritionRate()); continue;
         } 
         dataMap.put(entry.getKey(), Double.valueOf(((CustomDataDescribe)entry.getValue()).getAttritionRate().doubleValue() + ((Double)dataMap.get(entry.getKey())).doubleValue()));
       } 
     } 
