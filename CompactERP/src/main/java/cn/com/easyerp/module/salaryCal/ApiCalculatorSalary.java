...
public class ApiCalculatorSalary {
    ...
    public ApiResult test(@ApiParam("UUID") List<String> uuid, @ApiParam("date") List<String> dates) {
        ...
        try {
            ...
            Date lastDay = getLastDayOfMonth(Integer.valueOf(date.split("-")[0]).intValue(),
                    Integer.valueOf(date.split("-")[1]).intValue());
            ...
            double security = ((Double) sMap.get("s")).doubleValue() * ((Double) sMap.get("s1")).doubleValue()
                    * ((Double) sMap.get("s2")).doubleValue() * ((Double) sMap.get("s3")).doubleValue()
                    * ((Double) sMap.get("s4")).doubleValue();
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                for (int j = 0; j < holidays.size(); j++) {
                    holidayMoney += baseSalary * ((UserHoliday) holidays.get(i)).getHow_long()
                            * ((UserHoliday) holidays.get(i)).getPercent();
                }
                ...
                double preSalary = empInfo.getBase_salary() - lossMoney - lateMoney - holidayMoney + allDayMoney
                        + subsideMoney + performanceMoney;
    ...
    private Map<String, Integer> getAttendanceDetail(List<AttendanceJson> attendances, List<UserHoliday> holidays) {
        ...
        for (int i = 0; i < attendances.size(); i++) {
            ...
            if (holiday == null) {
                ...
                if (aJson.getWorkDate().equals(holiday.getBegin_date())
                        || aJson.getWorkDate().equals(holiday.getEnd_date())) {
                ...
                } else if (aJson.getWorkDate().compareTo(holiday.getBegin_date()) > 0
                        && aJson.getWorkDate().compareTo(holiday.getBegin_date()) < 0) {
                    type = 4;
                }
                ...
                if ("loss".equals(aJson.getStartClass())) {
                    ...
                    } else if ("early".equals(aJson.getEndClass()) && type != 2 && type != 3 && type != 4) {
                        lateOrEarly++;
                    }
