...
public class SettingService {
   ...
   public List<Map<String, Object>> getType(SqlRunner runner) {
	   ...
	   try {
		   result = runner.selectAll("select DXTypeCode, DXTypeName from U_DX_MASTER_TYPE where DXTypeName is not null and DXTypeName !=''", new Object[0]);
   ...
   public List<Map<String, Object>> queryList(String type) {
	   ...
	   dynamicSql = dynamicSql + ")SELECT  T1." + (String)((Map)typeResult.get(0)).get("DXCODEITEM") + " AS DXCODE, " + " REPLACE(T1." + (String)((Map)typeResult.get(0)).get("DXNAMEITEM") + ",' ','_') AS DXNAME, " + " T2.UTypeCode AS UTYPECODE," + " T3.UCode  AS UCODE,REPLACE(T3.UName,' ','_') AS UNAME, " + " T2.UACCCODE AS UACCCODE, " + "T4.UNAME AS UACCNAME" + " FROM " + " T1 " + " LEFT JOIN U_MASTER_MAPPING T2 ON T1." + (String)((Map)typeResult.get(0)).get("DXCODEITEM") + "=T2.DXCode " + " AND T2.DXTypeCode = '" + (String)((Map)typeResult.get(0)).get("DXTYPECODE") + "' " + " LEFT JOIN U_MASTER T3 ON T2.UCode=T3.UCode AND T3.UTypeCode='" + (String)((Map)typeResult.get(0)).get("UTYPECODE") + "' " + "LEFT JOIN U_MASTER T4 ON T4.UCode = T2.UAccCode AND T4.UTypeCode = 'U001' " + " ORDER BY T1." + (String)((Map)typeResult.get(0)).get("DXCODEITEM");
   ...
   public List<Map<String, Object>> queryUcode(String type) {
     ...
     dynamicSql = "SELECT * FROM U_MASTER WHERE UTypeCode='" + (String)((Map)typeResult.get(0)).get("UTYPECODE") + "' ORDER BY UCODE";
   ...
   public List<Map<String, Object>> querySubject(String searchid, String type) {
     ...
     dynamicSqlbuf.append(" (SELECT * FROM U_MASTER b  WHERE     b.UTypeCode = 'U001'              AND charindex (b.Ucode, a.UCode) = 1  AND LEN (b.ucode) = LEN (a.Ucode) - 2)");
   ...
   public Map<String, List<Map<String, Object>>> getTypeContent(String type) throws Exception {
     ...
     List<Map<String, Object>> dxResult = new ArrayList<Map<String, Object>>();
     sql = "select " + (String)((Map)typeResult.get(0)).get("DXCODEITEM") + " as code, " + (String)((Map)typeResult.get(0)).get("DXNAMEITEM") + " as name from " + (String)((Map)typeResult.get(0)).get("DXTABLE");
     try {
   ...
   public String updateSetting(String typeCode, String utypeCode, String data) {
     ...
     try {
       ...
       dynamicSql = "insert into U_MASTER_MAPPING(ID , DxTypeCode , DXCode , UTypeCode , UCode , UAccCode) values" + valuestr2;
       System.out.println("++++++++++" + dynamicSql);
