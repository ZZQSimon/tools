...
import cn.com.easyerp.module.dao.CustomReportDao;
 ...
 public class CustomReportService {
   ...
   public static SimpleDateFormat sdf = new SimpleDateFormat("yyyyMM"); @Autowired
   ...
   public List<CustomData> search(CustomReportRequestModel request, List<String> allMonth) {
     ...
     List<CustomData> customDataDescribes = this.customReportDao.selectData(changeCondition, countCondition, sumCondition, group, leavingType, timeBetween, isEmpType);
     ...
     for (CustomData data : datas) {
       ...
       for (CustomData customData : customDatas) {
         if ((data.getRegion() == null) ? (
           customData.getRegion() != null) : 
 
           
           !data.getRegion().equals(customData.getRegion())) {
           continue;
         }
         if ((data.getStore_Type() == null) ? (
           customData.getStore_Type() != null) : 
         ...
         if ((data.getGrade() == null) ? (
           customData.getGrade() != null) : 
 
           
           !data.getGrade().equals(customData.getGrade())) {
           continue;
         }
         if ((data.getEmployeeType() == null) ? (
           customData.getEmployeeType() != null) : 
 
           
           !data.getEmployeeType().equals(customData.getEmployeeType())) {
           continue;
         }
         if ((data.getLeavingType() == null) ? (
           customData.getLeavingType() != null) : 
         ...
         if ((data.getStore() == null) ? (
           customData.getStore() != null) : 
 
           
           !data.getStore().equals(customData.getStore())) {
           continue;
         }
         if ((data.getSum_grade() == null) ? (
           customData.getSum_grade() != null) : 
   ...
   private List<SumDescribe> buildSumCondition(FilterRequestModel filter) {
     ...
     allCount.setSum("SUM(IFNULL(t_monthly_staff_count.M2_actual, 0) + IFNULL(t_monthly_staff_count.M3_actual, 0) + IFNULL(t_monthly_staff_count.M4_actual, 0) + IFNULL(t_monthly_staff_count.P1_actual, 0) + IFNULL(t_monthly_staff_count.P2_actual, 0) + IFNULL(t_monthly_staff_count.P3_actual, 0) + IFNULL(t_monthly_staff_count.P4_actual, 0) + IFNULL(t_monthly_staff_count.PT_actual, 0))");
     ...
     if (null != filter.getFilters().get("Grade")) {
       ...
       for (int i = 0; i < values.size(); i++) {
         SumDescribe PTGrade; SumDescribe P4Grade; SumDescribe P3Grade; SumDescribe P2Grade; SumDescribe P1Grade; SumDescribe m4Grade; SumDescribe m3Grade; SumDescribe m2Grade; switch ((String)values.get(i)) {
           case "M2":
     ...
     if (values.size() == 2) {
       ...
       FTEType.setSum("SUM(IFNULL(t_monthly_staff_count.M2_actual, 0) + IFNULL(t_monthly_staff_count.M3_actual, 0) + IFNULL(t_monthly_staff_count.M4_actual, 0) + IFNULL(t_monthly_staff_count.P1_actual, 0) + IFNULL(t_monthly_staff_count.P2_actual, 0) + IFNULL(t_monthly_staff_count.P3_actual, 0) + IFNULL(t_monthly_staff_count.P4_actual, 0))");
     ...
     if ("FTE".equals(values.get(0))) {
       ...
       SumDescribe FTEType = new SumDescribe();
       FTEType.setGrade("FTE");
       FTEType.setSum("SUM(t_monthly_staff_count.M2_actual + t_monthly_staff_count.M3_actual + t_monthly_staff_count.M4_actual + t_monthly_staff_count.P1_actual + t_monthly_staff_count.P2_actual + t_monthly_staff_count.P3_actual + t_monthly_staff_count.P4_actual)");
