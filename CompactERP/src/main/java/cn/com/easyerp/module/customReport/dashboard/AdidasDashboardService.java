...
public class AdidasDashboardService {
    ...
    private static final String ONE_CHART = "one";
    private static final String TWO_CHART = "two";
    private static final String THREE_CHART = "three";
    private static final String FOUR_CHART = "four";
    private static final String FIVE_CHART = "five";
    private static final String SIX_CHART = "six";
    @Autowired
    ...
    public ActionResult diversitySelect(FilterRequestModel filter) {
        ...
        resultData.put(ONE_CHART, hcFteData);
        resultData.put(TWO_CHART, contractTypeData);
        resultData.put(THREE_CHART, lengthServiceData);
        resultData.put(FOUR_CHART, distriButtonData);
        resultData.put(FIVE_CHART, ageData);
        resultData.put(SIX_CHART, workYearData);
    ...
    private Map<String, Object> contractTypeSelect(FilterData filterData) {
        ...
        Map<String, Object> resultData = new HashMap<String, Object>();
        resultData.put(ONE_CHART, Integer.valueOf(0));
        resultData.put(TWO_CHART, Integer.valueOf(0));
        resultData.put(THREE_CHART, Integer.valueOf(0));
        ...
        for (int i = 0; i < data.size(); i++) {
            if (((Map) data.get(i)).get("contract_term_count") != null) {
                allCount = Integer.valueOf(allCount.intValue()
                        + Integer.parseInt(((Map) data.get(i)).get("contract_term_count").toString()));
            }
        }
        for (int i = 0; i < data.size(); i++) {
            if (((Map) data.get(i)).get("contract_term_count") != null) {

                String countFormat = paseCount(
                        Integer.parseInt(((Map) data.get(i)).get("contract_term_count").toString()),
                        allCount.intValue());
                if ("4".equals(((Map) data.get(i)).get("contract_term"))) {
                    resultData.put(ONE_CHART, countFormat);
                } else if ("6".equals(((Map) data.get(i)).get("contract_term"))) {
                    resultData.put(TWO_CHART, countFormat);
                } else if ("8".equals(((Map) data.get(i)).get("contract_term"))) {
                    resultData.put(THREE_CHART, countFormat);
                }
            }
        }
        return resultData;
    ...
    private Map<String, Object> workYearSelect(FilterData filterData) {
        ...
        if (boyData != null && boyData.size() != 0) {
            for (int i = 0; i < boyData.size(); i++) {
                int differMonth = differMonth((Date) ((Map) boyData.get(i)).get("entry_date"),
                        filterData.getBeginDate());
                if (differMonth <= 6) {
                    boyOne++;
                } else if (differMonth <= 12) {
                    boyTwo++;
        ...
        if (girlData != null && girlData.size() != 0) {
            for (int i = 0; i < girlData.size(); i++) {
                int differMonth = differMonth((Date) ((Map) girlData.get(i)).get("entry_date"),
                        filterData.getBeginDate());
                if (differMonth <= 6) {
                    girlOne++;
                } else if (differMonth <= 12) {
