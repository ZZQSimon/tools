...
public class Tree2Controller extends FormViewControllerBase {
    ...
    @SuppressWarnings({ "rawtypes" })
    public void getTreeDataUp(String item_id, String item_nm, String process_no, String lot_no, String p_item_id,
            String p_process_no, String p_lot_no, String parentId, String flag, String product_Detail_Id) {
        ...
        if (parentId.equals("") || parentId == null) {
            ...
            map.put("text", lotName + "(" + lot_no + ") 使用数("
                    + this.tree2Dao.userQuantityLotUpSelect(item_id, process_no, lot_no) + ")");
        } else {
            map.put("text", lotName + "(" + lot_no + ") 使用数("
                    + this.tree2Dao.userQuantityLotDownSelect(item_id, process_no, lot_no, product_Detail_Id) + ")");
        }
        ...
        if (rList.size() > 0) {
            ...
            for (Map<String, Object> rMap : rList) {
                ...
                String titleChNode = rMap.get("check_date").toString() + " " + okQuantity + "("
                        + rMap.get("ok_quantity").toString() + ") 銆�" + ngQuantity + "("
                        + rMap.get("ng_quantity").toString() + ") 銆�" + keepQuantity + "("
                        + rMap.get("keep_quantity").toString() + ")";
            ...
            map = new HashMap<String, String>();
            String textNode = this.dataService.getMessageText("检验", new Object[0]) + " " + okQuantity + "(" + sumOk
                    + ") 銆�" + ngQuantity + "(" + sumNg + ") 銆�" + keepQuantity + "(" + sumKeep + ")";
        ...
        if (rAndPList.size() > 0) {
            ...
            for (Map<String, Object> rPMap : rAndPList) {
                ...
                String titleChNode = rPMap.get("transReceive_date").toString() + " " + okQuantity + "("
                        + rPMap.get("ok_quantity").toString() + ") 銆�" + ngQuantity + "("
                        + rPMap.get("ng_quantity").toString() + ") 銆�" + keepQuantity + "("
                        + rPMap.get("keep_quantity").toString() + ")";
            ...
            map = new HashMap<String, String>();
            String textNode = this.dataService.getMessageText("接受/生产实绩", new Object[0]) + " " + okQuantity + "(" + sumOk
                    + ") 銆�" + ngQuantity + "(" + sumNg + ") 銆�" + keepQuantity + "(" + sumKeep + ")";
        ...
        if (pDList.size() > 0) {
            ...
            for (Map<String, Object> pDMap : pDList) {
                ...
                String tcn = countProductionDetail(pDMap.get("item_id").toString(), pDMap.get("process_no").toString(),
                        item_id, process_no, lot_no, flag);

                String titleChNode = pDMap.get("item_nm").toString() + "(" + pDMap.get("item_id").toString() + ") , "
                        + processName + "(" + pDMap.get("process_no").toString() + ")";
                map = new HashMap<String, String>();
                map.put("id", pDMap.get("item_id").toString() + pDMap.get("process_no").toString() + allLotNo);
                map.put("parent", "2" + this.stepAll);
                map.put("text",
                        titleChNode + " " + this.dataService.getMessageText("使用数合计", new Object[0]) + " (" + tcn + ")");
                this.TreeNode.add(map);
                allLotNo = "";
            }
        }
        ...
        if (fkList.size() > 0 || tkList.size() > 0) {
            ...
            for (Map<String, Object> fkMap : fkList) {
                ...
                String titleChNode = fkMap.get("judge_date").toString() + " " + okQuantity + "(-"
                        + fkMap.get("ok_quantity").toString() + ") 銆�" + ngQuantity + "("
                        + fkMap.get("ng_quantity").toString() + ") 銆�" + keepQuantity + "("
                        + fkMap.get("keep_quantity").toString() + ")";
                ...
                map = new HashMap<String, String>();
                map.put("id", fkMap.get("id").toString() + "t_judge_to_keep" + fkMap.get("judge_date").toString()
                        + this.stepAll);
            ...
            for (Map<String, Object> tkMap : tkList) {
                ...
                String titleChNode = tkMap.get("judge_date").toString() + " " + okQuantity + "(+"
                        + tkMap.get("ok_quantity").toString() + ") 銆�" + ngQuantity + "("
                        + tkMap.get("ng_quantity").toString() + ") 銆�" + keepQuantity + "("
                        + tkMap.get("keep_quantity").toString() + ")";
                map = new HashMap<String, String>();
                ...
                map.put("id", tkMap.get("id").toString() + "t_judge_from_keep" + tkMap.get("judge_date").toString()
                        + this.stepAll);
                map.put("parent", "3" + this.stepAll);
                map.put("text", titleChNode);
                this.TreeNode.add(map);
            }
            String textNode = this.dataService.getMessageText("保留判定", new Object[0]) + " " + okQuantity + "(" + sumOk
                    + ") 銆�" + ngQuantity + "(" + sumNg + ") 銆�" + keepQuantity + "(" + sumKeep + ")";
        ...
        if (sList.size() > 0) {
            ...
            for (Map<String, Object> sMap : sList) {
                ...
                if (sMap.get("adjust_type").toString().equals("1")) {
                    titleChNode = sMap.get("adjust_date").toString() + " " + adjustQuantity + "(+"
                            + sMap.get("adjust_quantity").toString() + ")";
                } else {
                    titleChNode = sMap.get("adjust_date").toString() + " " + adjustQuantity + "(-"
                            + sMap.get("adjust_quantity").toString() + ")";
                }
            ...
            String textNode = this.dataService.getMessageText("库存调整", new Object[0]) + " " + adjustQuantity + "("
                    + sumStock + ") ";
            map = new HashMap<String, String>();
        ...
        if (taList.size() > 0 || taAfterList.size() > 0) {
            ...
            for (Map<String, Object> taMap : taList) {
                String titleChNode = taMap.get("trans_date").toString() + " " + exportQuantity + "(-"
                        + taMap.get("export_quantity").toString() + ")";
            ...
            for (Map<String, Object> taAfterMap : taAfterList) {
                ...
                String titleChNode = taAfterMap.get("trans_date").toString() + " " + importQuantityAfter + "(+"
                        + taAfterMap.get("import_quantity_after").toString() + ")";
            ...
            String textNode = this.dataService.getMessageText("转账", new Object[0]) + " " + exportQuantity + "(" + sumEx
                    + ") " + " " + importQuantityAfter + "(" + sumIm + ") " + " ";
        ...
        for (Map<String, Object> pDMap : pDListLotNo) {
            ...
            allLotNo2 = "";
            if (flag.equals("Down")) {
                getTreeDataUp(pDMap.get("item_id").toString(), pDMap.get("item_nm").toString(),
                        pDMap.get("process_no").toString(), pDMap.get("lot_no").toString(), item_id, process_no, lot_no,
                        parentId, "Down", pDMap.get("product_Detail_Id").toString());
                continue;
            }
            getTreeDataUp(pDMap.get("item_id").toString(), pDMap.get("item_nm").toString(),
                    pDMap.get("process_no").toString(), pDMap.get("lot_no").toString(), item_id, process_no, lot_no,
    ...
    public String countProductionDetail(String item_id, String process_no, String p_item_id, String p_process_no,
            String p_lot_no, String flag) {
        // this.totalProD = 0.0F;
        float sum = 0.0F;
        if (flag.equals("Down")) {
            if (this.tree2Dao.userQuantityDownSelect(item_id, process_no, p_item_id, p_process_no, p_lot_no) != null) {
                sum = Float.parseFloat(
                        this.tree2Dao.userQuantityDownSelect(item_id, process_no, p_item_id, p_process_no, p_lot_no));
            }
        } else {
