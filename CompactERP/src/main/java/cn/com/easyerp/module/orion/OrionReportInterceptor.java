...
public class OrionReportInterceptor extends ReportServiceDefault<OrionReportParam> {
    ...
    public void init(final ReportModel<OrionReportParam> report) {
        super.init(report);
        report.setParam((OrionReportParam) Common.fromJson(report.getService_param(),
                (TypeReference) OrionReportInterceptor.OrionReportModelRef));
    }
    ...
    @Override
    public ActionResult print(final ReportModel<OrionReportParam> report, final String where, final AuthDetails user,
    ...
    private static class LabelPrinter implements Printable {
        ...
        public int print(final Graphics g, final PageFormat pf, final int index) throws PrinterException {
            ...
            final byte[] img = QRCode.from(detail.export(this.param)).withHint(EncodeHintType.MARGIN, (Object) 0)
                    .withSize(this.param.getLabelSize(), this.param.getLabelSize()).to(ImageType.PNG).stream()
                    .toByteArray();
            final int left = 90;
            int h = 10;
            final int lh = 15;
