...
import cn.com.easyerp.ril.ReferenceInputListFormRequestModel;
 ...
 public class AutoGenerateController
   ...
   private GridService gridService; @Autowired
   private ComplexInputService complexInputService; @Autowired
   private DxToolService dxToolService; public ModelAndView batch(TableDescribe table, AutoGenTableDesc ag, String parent) {
     ...
     for (ColumnDescribe column : table.getColumns()) {
       ...
       if ("_SYS_COLUMN".equals(column.getGroup_name()) || table
         .getIdColumns()[0].equals(columnName) || column
         .getData_type() == 13)
         continue; 
       columns.add(columnName);
       if (view.getColumn(columnName) != null && 
         !view.getColumn(columnName).isRo_insert()) {
         inputs.add(columnName);
       }
     } 
     
     AutoGenerateFormModel form = new AutoGenerateFormModel(parent, ag.getRef_view(), ag, filter, grid, hasSum, columns, inputs);
   ...
   public ActionResult list(@RequestBody AutoGenerateFormRequestModel request) {
     ...
     for (Map.Entry<String, Object> entry : request.getFilterData().entrySet()) {
       ColumnDescribe column; String name = (String)entry.getKey();
     ...
     List<Map<String, Object>> data = this.dataService.dynamicQueryList(form.getAg().getGen_sql(), request
         .getFilterData());
     ...
     for (Map<String, Object> recordData : data) {
       ...
       for (FieldModelBase field : fields) {
         ...
         if (form.getInputs().contains(field.getColumn()))
           tagMap.put(field.getId(), this.dxToolService.field(field, form)); 
       } 
