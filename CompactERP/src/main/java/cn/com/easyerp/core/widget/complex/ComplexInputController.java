...
import cn.com.easyerp.framework.enums.ActionType;
 ...
 public class ComplexInputController
   extends DetailForm
 {
   @Autowired
   ...
   protected DetailFormService getBuilder() { return this.builder; }
 
 
   
   private List<RecordModel> buildRecords(List<Map<String, Object>> list, TableDescribe table, ComplexColumnDescribe complex, GridModel grid) {
     if (list == null)
       return null; 
     ...
     for (Map<String, Object> map : list) {
       ...
       if (!complex.extended) {
         ...
         for (Map.Entry<String, Object> entry : map.entrySet()) {
           ...
           if (found)
             continue;  amount = entry.getValue();
         } 
   ...
   private List<? extends RecordModel> initStd(ComplexInputRequestModel request, ComplexInputFieldModel field, ComplexColumnDescribe complex, GridModel grid) {
     ...
     if (list.size() == 0) throw new ApplicationException(this.dataService.getMessageText("no reference data", new Object[0]));
   ...
   private List<? extends RecordModel> initExtended(ComplexInputRequestModel request, ComplexInputFieldModel field, ComplexColumnDescribe complex, GridModel grid) {
     ...
     for (ColumnDescribe column : columns) {
       ...
       if (name.equals(complex.getDetail_tbl_pid_col()) || name
         .equals(complex.getDetail_tbl_pnm_col()) || name
         .equals(complex.getDetail_tbl_number_col()) || name
         .equals("is_old"))
         continue; 
     ...
     if (request.getAction() == ActionType.edit) {
       List<Map<String, Object>> list = this.dataService.selectRecordsByValues(tableName, field
           .allColumnNames(), request.getParam());
       field.setDetails(list);
     } 
     return buildRecords(field.getDetails(), detailTable, complex, grid);
   }
   ...
   public ModelAndView dialog(@RequestBody ComplexInputRequestModel request) {
     ...
     ComplexInputFormModel form = new ComplexInputFormModel(request.getAction(), id, desc.getTable_id(), desc.getColumn_name(), field.getColumnNames(), complex, grid);
   ...
   public ActionResult save(@RequestBody ComplexInputRequestModel request) {
     ...
     for (RecordModel record : grid.getRecords()) {
       if (record.getStatus() == RecordModel.Status.deleted)
         continue;  Map<String, Object> map = new HashMap<String, Object>(record.getFields().size());
       for (FieldModelBase f : record.getFields()) {
         String column = f.getColumn();
         if (!desc.isExtended() && column.equals(detailColumn)) {
           map.put(column, request.getParam().get(record.getId())); continue;
         } 
