...
public class MapSelectService {
    ...
    private void doUpdate(TableDescribe table, Map<String, FieldModelBase> fields, Map<String, Object> data,
            UpdateType type) {
        for (ColumnDescribe column : table.getColumns()) {
            ...
            String value = (type == UpdateType.delete) ? (String) data.get(columnName)
                    : (((MapFieldModel) fields.get(columnName)).isModified()
                            ? (String) ((FieldModelBase) fields.get(columnName)).getValue()
                            : null);
            if (value == null)
                continue;
    ...
    public void inspectInsert(TableDescribe table, Map<String, FieldModelBase> fields) {
        doUpdate(table, fields, null, UpdateType.insert);
    }

    public void inspectUpdate(TableDescribe table, Map<String, FieldModelBase> fields) {
        doUpdate(table, fields, null, UpdateType.update);
    }

    public void inspectDelete(TableDescribe table, Map<String, Object> data) {
        doUpdate(table, null, data, UpdateType.delete);
    }
}
