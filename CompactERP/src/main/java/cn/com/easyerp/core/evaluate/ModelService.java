...
public class ModelService {
    ...
    public void clearListModel(String id) {
        this.cacheModelService.clearListModel(id);
    }
    ...
    private Map<String, ChildModel> buildChildrenModel(List<GridModel> children) {
        ...
        for (int i = 0; i < children.size(); i++) {
            ...
            ChildModel child = new ChildModel(((GridModel) children.get(i)).getId(),
                    ((GridModel) children.get(i)).getParent(), ((GridModel) children.get(i)).getTable());
    ...
    private List<DataModel> buildChildDatas(List<RecordModel> datas) {
        ...
        for (int i = 0; i < datas.size(); i++) {
            DataModel dataModel = new DataModel(((RecordModel) datas.get(i)).getId(),
                    ((RecordModel) datas.get(i)).getParent());
