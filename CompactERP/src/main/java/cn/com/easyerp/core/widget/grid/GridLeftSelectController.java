...
import cn.com.easyerp.tree.TreeNodeModel;
 ...
 public class GridLeftSelectController
 {
   ...
   private CacheService cache; @Autowired
   ...
   private void getLeftSelectRecodes(WidgetModelBase form, TableDescribe table, String column, GridLeftModel glm) {
     ...
     if (groupColumn.getDic_id() != null && !"".equals(groupColumn.getDic_id())) {
       ...
       if (parent_id_column != null && !"".equals(parent_id_column) && children_id_column != null && 
         ...
         treeNodeModels = dumpTreeNodes(form, refTable, this.systemDao.selectViewGridValue(refTable, null), column);
       } else {
         ...
         treeNodeModels = dumpNodes(form, refTable, this.systemDao.selectViewGridValue(refTable, where), column, true);
       } 
       glm.setLeftNodes(treeNodeModels);
       glm.setType("3");
     } else if (groupColumn.getDic_id() == null || "".equals(groupColumn.getDic_id()) || groupColumn
       .getRef_table_name() == null || "".equals(groupColumn.getRef_table_name())) {
   ...
   private List<TreeNodeModel> dumpTreeNodes(WidgetModelBase form, TableDescribe table, List<Map<String, Object>> list, String column) {
   ...
   private List<TreeNodeModel> dumpNodes(WidgetModelBase form, TableDescribe table, List<Map<String, Object>> list, String column, boolean is_ref) {
     List<TreeNodeModel> nodes = new ArrayList<TreeNodeModel>();
     for (Map<String, Object> map : list) {
       String childrenId;
   ...
   private void buildTreeNodeSelected(GridLeftModel glm, FilterDescribe filter) {
     if (filter == null || filter.getValue() == null || glm
       .getLeftNodes() == null || glm.getLeftNodes().size() == 0) {
       return;
     }
