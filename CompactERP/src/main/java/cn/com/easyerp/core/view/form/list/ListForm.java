...
@Controller
@RequestMapping({ "/list" })
public class ListForm extends FormViewControllerBase {
    @Autowired
    ...
    public ModelAndView view(@RequestBody ListFormRequestModel request, AuthDetails user) {
        ...
        String filterTable = (table.getViewStyle() != null && table.getViewStyle().getFilter() != null)
                ? table.getViewStyle().getFilter().getReference()
                : tableName;

        FilterModel filter = new FilterModel(filterTable, "Search", null, request.getFixedData(),
                request.getDefaultFilters());
        Set<TableViewStyle.Button> hideButtons = request.getHideButtons();

        if (table.asView() && request.getAction() != ActionType.select) {
            form = ViewListForm.create(request.getParent(), tableName, filter, grid, hideButtons, request.isLoad(),
                    request.isControl());
        } else {
            form = createForm(request.getAction(), request.getParent(), tableName, filter, grid, hideButtons,
                    request.isLoad(), request.isControl());
        }
    ...
    protected ListFormModel createForm(ActionType action, String parent, String tableName, FilterModel filter,
        ...
        return new ListFormModel(action, parent, tableName, filter, grid, hideButtons, load, control);
    }
