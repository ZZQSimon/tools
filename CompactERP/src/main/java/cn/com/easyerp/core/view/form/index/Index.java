...
import cn.com.easyerp.framework.common.Common;
   ...
   extends FormViewControllerBase
 {
   ...
   public ModelAndView index(AuthDetails user, HttpSession session) throws ParseException {
     ...
     form.setSysCompanyName((session.getAttribute("SysCompanyName") == null) ? "" : session.getAttribute("SysCompanyName").toString());
     form.setSysLogo((session.getAttribute("SysLogo") == null) ? "" : session.getAttribute("SysLogo").toString());
     form.setInterior_logo((session.getAttribute("interior_logo") == null) ? "" : session.getAttribute("interior_logo").toString());
     ...
     return buildModelAndView(form); } @Autowired private IndexCacheService indexCacheService; @Autowired private DxToolService dxToolService; @Autowired
   private ApproveService approveService; @Autowired
   private SystemDao systemDao; @Autowired
   private DashboardService dashService; @RequestMapping({"/selectApproveDatas.view"})
   public ModelAndView selectApproveDatas(AuthDetails user) { IndexApproveModel form = new IndexApproveModel();
   ...
   public MatterDeskTopModel matterItem(AuthDetails user) {
     ...
     for (IndexStatusModel data : datas) {
       ...
       for (OperationDescribe op : ops) {
         if (op.isBacklog_event()) {
           ...
           if (this.authService.optionAuth(user, op.getTable_id(), op.getId(), u
               .getId()))
     ...
     for (Map.Entry<String, Map<String, Map<String, IndexStatusModel>>> tableEntry : dataMap.entrySet()) {
       ...
       List<Map<String, Object>> dataList = this.dataService.selectRecordsByValues(tableName, null, (Map)null, where);
   ...
   public ActionResult reloadTodo(AuthDetails user, @RequestBody FormRequestModelBase<IndexModel> request, HttpServletRequest req, HttpServletResponse resp) {
   ...
   public DataModel desktop(List<DesktopItemModel> desktops, AuthDetails user, boolean personal) throws ParseException {
     int heigthY = 145;
     int widthX = 245;
     int maxRow = 0;
     ...
     for (DesktopItemModel desktop : desktops) {
       ...
       if (!desktop.getBulletin_id().isEmpty()) {
         ...
         if (bull != null) {
           ...
           bull.setHeight((bull
               .getEnd_row() - bull.getStart_row() + 1) * heigthY + HeightAndWidth(bull
                 .getEnd_row(), bull.getStart_row()));
           bull.setWidth((bull
               .getEnd_col() - bull.getStart_col() + 1) * widthX + HeightAndWidth(bull
                 .getEnd_col(), bull.getStart_col()));
         ...
         Map<String, Object> relist = this.indexService.getReList(userId, desktop
             .getDate_reminder_id());
         if (relist != null) {
           ...
           relist.put("height", Integer.valueOf((((Integer)relist.get("end_row")).intValue() - ((Integer)relist.get("start_row")).intValue() + 1) * heigthY + 
                 HeightAndWidth(((Integer)relist
                   .get("end_row")).intValue(), ((Integer)relist.get("start_row")).intValue())));
           relist.put("width", Integer.valueOf((((Integer)relist.get("end_col")).intValue() - ((Integer)relist.get("start_col")).intValue() + 1) * widthX + 
                 HeightAndWidth(((Integer)relist.get("end_col")).intValue(), ((Integer)relist
                   .get("start_col")).intValue())));
         ...
         if (chart != null) {
           ...
           chart.setHeight((chart
               .getEnd_row() - chart.getStart_row() + 1) * heigthY + HeightAndWidth(chart
                 .getEnd_row(), chart.getStart_row()));
           chart.setWidth((chart
               .getEnd_col() - chart.getStart_col() + 1) * widthX + HeightAndWidth(chart
                 .getEnd_col(), chart.getStart_col()));
         ...
         if (shct != null) {
           ...
           shct.setHeight((shct
               .getEnd_row() - shct.getStart_row() + 1) * heigthY + HeightAndWidth(shct
                 .getEnd_row(), shct.getStart_row()));
           shct.setWidth((shct
               .getEnd_col() - shct.getStart_col() + 1) * widthX + HeightAndWidth(shct
                 .getEnd_col(), shct.getStart_col()));
         ...
         matter.setHeight((desktop
             .getEnd_row() - desktop.getStart_row() + 1) * heigthY + HeightAndWidth(desktop
               .getEnd_row(), desktop.getStart_row()));
         matter.setWidth((desktop
             .getEnd_col() - desktop.getStart_col() + 1) * widthX + HeightAndWidth(desktop
               .getEnd_col(), desktop.getStart_col()));
   ...
   public Map<String, Object> listReset(Map<String, Object> relist) {
     List<Map<String, Object>> listReminder = this.indexService.getSuserList(relist
         .get("table_id").toString(), relist.get("column_name")
         .toString(), relist.get("name_column").toString(), relist
         .get("id").toString(), relist.get("type").toString());
   ...
   public WorkModel workReset(WorkModel work) throws ParseException {
     ...
     work.setHeight((work
         .getEnd_row() - work.getStart_row() + 1) * height + HeightAndWidth(work.getEnd_row(), work
           .getStart_row()));
     work.setWidth((work.getEnd_col() - work.getStart_col() + 1) * width + HeightAndWidth(work.getEnd_col(), work
           .getStart_col()));
     ...
     work.setSql_Number(bigsql.setScale(2, 4)
         .toString());
   ...
   public ActionResult grid(@RequestBody Map<String, Object> param) {
     ...
     String statusName = this.indexDao.selectStatus(param.get("column_name").toString(), tableName, key, param
         .get("key_value").toString());
