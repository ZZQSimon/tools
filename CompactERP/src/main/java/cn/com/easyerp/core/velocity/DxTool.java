...
import cn.com.easyerp.operation.OperationFormModel;
 ...
 public class DxTool {
   ...
   private SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm"); private boolean jsDebug; private String errorMsg;
   ...
   public String json(Object value) { return Common.toJson(value); }
   ...
   public String label(TableShortcutDescribe shortcut) { return this.service.dataService.getLabel(shortcut); }
   ...
   public Map<String, I18nDescribe> dict(String key1) { return this.service.dataService.getDict(key1); }
   ...
   public List<DictNameDescribe> getReports() {
     ...
     for (Map.Entry<String, I18nDescribe> entry : reports.entrySet()) {
       ...
       oneDict.setDictText((this.service.dataService.i18nString((I18nDescribe)entry.getValue()) == null) ? "" : this.service.dataService.i18nString((I18nDescribe)entry.getValue()));
       dictList.add(oneDict);
     } 
     DictNameDescribe oneDict = new DictNameDescribe();
     oneDict.setDictKey("");
   ...
   public String text(MapFieldModel field) {
     ...
     return msg("w.map.selected", new String[] {
           String.valueOf(selection.size()), 
           Common.notBlank((String)entry.getValue()) ? (String)entry.getValue() : (String)entry.getKey()
         });
   }
   
   public AuthDetails getUser() { return AuthService.getCurrentUser(); }
   ...
   public List<ColumnDescribe> columns(String tableName) { return this.service.dataService.getTableDesc(tableName).getColumns(); }
   ...
   public TableDescribe table(String tableName) { return this.service.dataService.getTableDesc(tableName); }
   
   public ColumnDescribe desc(FieldModelBase field) { return this.service.dataService.getColumnDesc(field); }
   ...
   public ColumnDescribe desc(String table, String column) { return this.service.dataService.getColumnDesc(table, column); }
   
   public boolean visible(TableGroupDescribe group) { return (!Common.isBlank(group.getId()) && !DataService.isSystemGroup(group.getId())); }
   ...
   public boolean visible(FormModelBase form, ColumnDescribe desc) {
     ...
     if (OperationFormModel.class.isInstance(form)) {
       ...
       api = (ApiDescribe)this.service.dataService.getTrigger(opForm.getOperationId()).getApi().get(opForm.getSeq());
     }
     ...
     if (table.getViewStyle() != null && table.getViewStyle().getSeq() != null && table.getViewStyle().getSeq().equals(columnName)) {
       return false;
     }
     ...
     } else if (table.getParent_id() != null && table
       .getId_column().contains("" + columnName + "")) {
       return false;
     } 
     return true;
   }
   
   public List<String> ds() { return this.service.dxRoutingDataSource.getList(); }
   ...
   public List<String> children(TableDescribe table) { return this.service.dataService.getChildrenTable(table.getId()); }
   
   public OperationDescribe operation(String oid) { return this.service.dataService.getOperation(oid); }
   
   public TableShortcutDescribe ts(String sid) { return this.service.dataService.getTableShortcut(sid); }
   
   public String[] split(String str, String splitter) { return Common.split(str, splitter); }
   
   public Set<String> getJs() throws IOException { return this.service.jsList; }
   
   public Set<String> getCss() throws IOException { return this.service.cssList; }
   
   public boolean auth(AuthDetails user, TableShortcutDescribe ts) { return this.service.authService.newAuth(user, ts.getId()); }
 
   public boolean auth(AuthDetails user, TableDescribe table) { return true; }
   
   public boolean exportAuth(AuthDetails user, String table, boolean isSelected) { return this.service.authService.exportAuth(user, table, null, isSelected); }
   
   public boolean importAuth(AuthDetails user, String table, boolean isInsert) { return this.service.authService.importAuth(user, table, null, isInsert); }
   ...
   public boolean addAuth(AuthDetails user, TableDescribe table, String actionId) {
     ...
     if (triggers == null || triggers.get(actionId) == null || ((TriggerDescribe)triggers.get(actionId)).getIs_using() != 1)
       return false; 
     try {
   ...
   public boolean editAuth(AuthDetails user, TableDescribe table, Map<String, FieldModelBase> fieldMap) {
     ...
     try {
       ...
       this.service.authService.assertAuth(user, table.getId(), table
           .getTrigger(TriggerDescribe.TriggerType.edit).getAction_id(), owner);
     } catch (Exception e) {
       return false;
     } 
     return true;
   }
   public boolean operationButtonAuth(Map<String, FieldModelBase> fieldMap, AuthDetails user, TableDescribe table, String operation) {
     String owner = "";
   ...
   public FieldModelBase field(String id) { return (FieldModelBase)ViewService.fetchWidgetModel(id); }
   ...
   public boolean required(ColumnDescribe column) {
     ...
     return (column.getMin_len() == null || column.getMin_len().intValue() > 0);
   }
   
   public boolean isJsDebug() { return this.jsDebug; }
   
   public void cacheFormModel(FormModelBase form) { this.service.cacheFormModel(form); }
   
   public boolean filter(TableDescribe table, ColumnDescribe desc) {
     return (!desc.isVirtual() && desc
       .getIs_condition().intValue() >= 0 && desc
       .getData_type() != 6 && (table
       .getRangeList() == null || !table.getRangeList()[1].equals(desc.getColumn_name())));
   }
   public boolean hasFilter(TableDescribe table) {
     ...
     for (ColumnDescribe column : columns) {
       if (!column.isVirtual() && column.getIs_condition().intValue() == 1 && column
         .getData_type() != 6 && (table
         .getRangeList() == null || !table.getRangeList()[1].equals(column.getColumn_name())))
         return true; 
     } 
     return false;
   }
   ...
   public boolean isMap(Object o) { return Map.class.isInstance(o); }
   
   public boolean isMapColumn(FieldModelBase field) { return this.service.dataService.isMapColumn(desc(field)); }
   ...
   public boolean selectIsAddApprove(String approve_id, String block_id) { return this.service.dataService.selectIsAddApprove(approve_id, block_id); }
   ...
   public List<FlowEvent> approveButton(String tableName, ApproveFlowNode node) { return approveButton(tableName, node.getBlock_id(), node.getBelong_block()); }
   ...
   public boolean approveBlockIsExec(ApproveFlow approveFlow, String block_id, String flow_event_id) {
     if (null == approveFlow.getBlockEventIsExec() || null == approveFlow.getBlockEventIsExec().get(block_id))
       return false; 
     ApproveBlockEventExec approveBlockEventExec = (ApproveBlockEventExec)((Map)approveFlow.getBlockEventIsExec().get(block_id)).get(flow_event_id);
   ...
   public boolean ifAllowUndo(String approveId) { return (this.service.ifAllow(approveId) == 0); }
