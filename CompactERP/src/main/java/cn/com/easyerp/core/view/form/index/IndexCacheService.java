...
@Service
public class IndexCacheService extends CacheLoader<List<OperationDescribe>> {
    ...
    public Map<String, List<OperationDescribe>> reload() {
        ...
        for (OperationDescribe op : this.cacheService.getOperations().values()) {
            String key = Common.makeMapKey(op.getTable_id(), op.getColumn_name(),
                    new String[] { op.getStatus_id_from() });
