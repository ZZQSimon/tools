...
import cn.com.easyerp.framework.common.ActionResult;
   ...
   extends FormViewControllerBase
 {
   ...
   public ActionResult setAttendanceArea(AuthDetails auth, @RequestBody MapRequestModel request) {
     ...
     try {
       ...
       if (areas != null && areas.size() > 0) {
         ...
         this.attendanceDao.saveAttendanceArea(attendanceId, request.getAttendanceName(), request
             .getAttendanceArea(), request.getAttendanceLon(), request.getAttendanceLat(), user_id, date);
       }
   ...
   public ActionResult updateAttendanceTime(@RequestBody MapRequestModel request) {
     String result = "success";
     try {
       this.attendanceDao.updateAttendanceTime(request.getBeginTime(), request.getEndTime(), request
           .getAttendanceId());
     } catch (Exception e) {
