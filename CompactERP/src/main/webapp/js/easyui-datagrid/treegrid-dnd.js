		...
		resetDnd: function(jq){
			return jq.each(function(){
				...
				if (row){
					...
					tr.each(function(){
						var target = this;
						$(target).data('__draggable').droppables = $('.__droppable:visible').filter(function(){
							return target != this;
						}).filter(function(){
							var accept = $.data(this, '__droppable').options.accept;
							if (accept){
			...
			return jq.each(function(){
				...
				nodes.__draggable({
					...
					proxy: function(source){
						...
						p.html('<span class="tree-dnd-icon tree-dnd-no">&nbsp;</span>' + 'tree');
						p.hide();
						return p;
					},
					deltaX: 15,
					...
					onDrag:function(e){
						...
						if (d>3){	// when drag a little distance, show the proxy object
							...
							e.data.startX = tr.offset().left;
							e.data.startY = tr.offset().top;
				...
				function setValid(id, valid){
					...
					tr.__droppable({accept:accept});
					tr.next('tr.treegrid-tr-tree').find('tr[node-id]').__droppable({accept:accept});
				}
			});
		}
	});
