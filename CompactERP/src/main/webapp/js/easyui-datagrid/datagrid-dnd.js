			...
			return jq.each(function(){
				...
				var draggableOptions = {
					...
					proxy: function(source) {
						...
						$.map(rows, function(row,i){
							...
							tr2.clone().removeAttr('id').removeClass('__droppable').appendTo(p);
							tr1.clone().removeAttr('id').removeClass('__droppable').find('td').insertBefore(p.find('tr:eq('+i+') td:first'));
					...
					onEndDrag: function(e){
						var dd = $(this).data('__draggable').droppables.filter(function(){
							var dropObj = $(this);
							if (dropObj.__droppable('options').disabled){return false;}
						...
						$(this).data('__draggable').droppables = dd;
					},
					onStopDrag:function(){
				...
				var droppableOptions = {
					...
					onDragEnter: function(e, source){
						if ($(this).__droppable('options').disabled){return;}
					...
					onDragOver: function(e, source) {
						if ($(this).__droppable('options').disabled){
							return;
						}
				...
				trs.__draggable(draggableOptions);
				trs.__droppable(droppableOptions);
				setDroppable(target);
				
				function setProxyFlag(source, allowed){
					var icon = $(source).__draggable('proxy').find('span.tree-dnd-icon');
				...
				function setDroppable(target){
					getDroppableBody(target).__droppable(droppableOptions).__droppable('enable');
				}
				...
				function isValid(tr){
					var opts = $(tr).__droppable('options');
				...
				function setValid(rows, valid){
					...
					$.map($.isArray(rows)?rows:[rows], function(row){
						...
						opts.finder.getTr(target, index).__droppable({accept:accept});
					});
				}
			});
		}
