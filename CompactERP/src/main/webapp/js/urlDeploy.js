 ...
 */
registerInit('Url', function (form) {

    init();
    var dataGrid = form.get().find('#urlDeployTb');
    form.get().find(".parameterInput").on("click",function(){
	   	var josnstr=form.get().find(".parameterInputControl").val();
	   	var paramId="44";
	   	var modalX="44";
	   	var modalY="44";
	   	form.get().find(".parameterInput").parameterInputControl(true).setData("onSubmit",function(data){
	   		form.get().find(".parameterInputControl").val(data);
    	},josnstr,paramId,modalX,modalY);
    	
    });

    function init() {
        postJson('/url/findAllUrl.do', {}, function (data) {
            if (data) {
                var totlerow = {
                    "total": data.count,
                    "rows": data.list
                ...
                var c_url = dx.table.c_url;
                var column = c_url.columns;
                var row;
    ...
    form.get().find(".easyui-linkbutton.append").on("click", function () {
        append();
    });
    form.get().find(".easyui-linkbutton.removeit").on("click", function () {
        removeit();
    });
    form.get().find(".easyui-linkbutton.accept").on("click", function () {
        accept();
    });
    form.get().find(".easyui-linkbutton.reject").on("click", function () {
        reject();
    });
