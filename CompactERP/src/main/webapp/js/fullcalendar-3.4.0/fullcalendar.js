			...
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
                        ...
                        var selectHtml = '';
                        var i = 0;
                        selectHtml +="<span class='fc-dateSelect fc-header-title'>";
                        selectHtml +="<select name='fcs_date_year' class='fcs_date_year selectable m_year mr15'>";
                        // 循环年份
                        for(i=2010;i<=2050;i++){
                        ...
                        selectHtml +="<select name='fcs_date_month' class='fcs_date_month selectable m_year'>";
                        // 循环月份
                        var monthDigitCN = ['一', '二', '三', '四', '五', '六', '七', '八', '九', '十', '十一', '十二'];
                        for(i=1;i<=12;i++){
                            selectHtml +="<option value='"+i+"'>" + $.trim(monthDigitCN[i-1]+"月") + "</option>";
                        }
                        selectHtml +="</select>";
                        selectHtml +="</span>";
                        sectionEl.append(selectHtml);
                        if (prevButton) {
	...
	function updateTitle(text) {
		if (el) {
            ...
            if(shtm && shtm.length>1){
                el.find(".fcs_date_month").find("option").filter(function(){
                    // if($(this).text() == $.trim(shtm[1])){
                     //    console.log($(this).text());
                     //    console.log($.trim(shtm[1]))
					// }
                    return ($(this).text() == $.trim(shtm[1]));
                }).prop('selected', true);
                el.find('.fcs_date_year option[value="'+$.trim(shtm[0]).replace(/[^0-9]/ig,"")+'"]').prop('selected', true);
            }
        }
	}
